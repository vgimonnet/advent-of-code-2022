...
const buildGrid = () => {
  for (let i = 0; i < nbRows + 1; i++) {
    let row = [];
    for (let j = 0; j < nbColumns + 1; j++) row.push('.');
    rows[i] = row;
  }
...
const getMoves = (x, y) => ({
  down: { x: x, y: y + 1, val: rows[y + 1] !== undefined ? rows[y + 1][x] : undefined },
  left: { x: x - 1, y: y + 1, val: rows[y + 1] !== undefined ? rows[y + 1][x - 1] : undefined },
  right: { x: x + 1, y: y + 1, val: rows[y + 1] !== undefined ? rows[y + 1][x + 1] : undefined },
});
